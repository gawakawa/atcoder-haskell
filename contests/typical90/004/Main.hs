...
import Data.Array.Unboxed (UArray, (!), bounds, elems, listArray, accumArray, accum)
...
rowSums mat = 
    accumArray (+) 0 (both fst $ bounds mat)
        [ (r, mat ! (r, c)) 
        | r <- range $ both fst $ bounds mat
        , c <- range $ both snd $ bounds mat
        ]
...
colSums mat = 
    accumArray (+) 0 (both snd $ bounds mat)
        [ (c, mat ! (r, c)) 
        | r <- range $ both fst $ bounds mat
        , c <- range $ both snd $ bounds mat
        ] 
